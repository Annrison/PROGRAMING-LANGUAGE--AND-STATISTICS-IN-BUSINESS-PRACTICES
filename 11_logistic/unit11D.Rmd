---
title: UNIT11D：回歸的係數判讀 Regression Coeffieient 
subtitle: 案例一：預防性醫療
author: 中山大學管理學院 卓雍然
date: "`r Sys.time()`"
output: 
...
+ 基礎機率
建模之前，看看PoorCare的比率有多少，大概是1/4
基礎模型base-line；只有y的話要猜y的機率，做模型希望可以比base-line準，y在這裏的話75%是0
```{r}
...
set.seed(88)
# split 是一個 vector
split = sample.split(D$PoorCare, SplitRatio = 0.7)  
# split 分割的資料集PoorCare的比率都會一樣
...
線性模型好用是因為可以直接用x的係數看出x變量對y的邊際效果，如果是邏輯式回歸，要預測的是y=1的機率，但因為要經過轉換所以對x的影響沒有那麼直覺。不過還是可以從係數知道，如果某個x的係數是正的且顯著的話，那對y=1的機率有正向關係。

![title](img/3.png)
邏輯式回歸裡面，係數取exp()之後的值，
就是x每增加1的時候，y=1的勝率會增加的倍數。
...
【???】
k是增加的倍數，po是原本的機率，p1是乘以兩倍之後的機率。</br>
這裡可以發現，給定勝率的比，機率的增加不一定會一樣。
<br><br>

**變數的邊際效果** 
當`OfficeVisits`和`Narcotics`分別都等於它們的中位數時，$y=1$的機率為

要算變數的邊際效果的時候，需要把變數固定在一個值。
```{r}
