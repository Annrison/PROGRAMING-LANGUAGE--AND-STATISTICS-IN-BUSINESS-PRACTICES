...
分析模擬 {data-orientation=rows}
=====================================

Row {data-height=75%}
-------------------------------------
### 變數分布 & 機率(密度)函數 {data-width=70%}
```{r}
renderPlot({
  par(mar=c(4,4,2,1))
  plot(0,0,xlim=c(1.5,5.25),ylim=c(0,1.1),xlab="Waiting Time", 
       ylab="Desnity/Probability", main="Distribution")
...
sliderInput("R", "數值範圍", 1, 6, c(1,1), 0.1, width='400px')
...
[教學影片](https://www.youtube.com/embed/PsV0h3O0Gc4)
(ctrl + click)
...
<p class="wwl">
【A】從本周的課程我們學到使用`頻率`、`密度`和`機率`來表示`隨機變數`的`分布`<br>
