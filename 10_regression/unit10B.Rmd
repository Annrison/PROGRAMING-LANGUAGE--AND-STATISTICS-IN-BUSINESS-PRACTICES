...
```{r}
#md是我們的模型，用list儲存
#Intercept就是截距(剛剛那個係數b_0)
#Grocery那欄是Grocery變化對Milk的變動量b_1
summary(md)
```
...
<span style="font-size:24px"> `r "\U1F4A1"` : </span>
係數($b_0, b_1$)也都是隨機變數
上面那個圖是在說b_0的分佈有95信心區間會在這個區域裡面
大概在0.69到0.79裡面，我們有95%的信心可以說變數b_0會在這個區間裡面
<br>
...
如果用模型解釋他自己，error就是粉紅色的那個</br>
如果是用x去預測的話error會是藍色的</br>
其中的差值就是模型解釋掉的部分(R^2)，可以拿來當準確度指標</br>
...
x = "dose", y = "len"
x是類別變數，每一個類別都會估計一個y的參考值。
來看每一個x之間的y有沒有顯著差異
這個的前提是須要是一個簡單線性模型，就是y需要是連續的
